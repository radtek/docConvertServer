...
bool ChangeImgName(string imgPath, int fileid)
{
...
int ConvertPdfToImg(const char *sourcefile, const char *outpath, const int &fileid)
{
	int nReturn = 3;
	...
	int nToImgStatusOk = 0;
	int nToImgStatusError = 0;
		...
		for (i; i<1; i++)
		{
			...
			if (len > 0)
			{
				...
				if (CheckCodeIsUnrecognizable(c_buf))
				{
					...
					int bhtml = ExtractPdftoPng(sourcefile, imgfile, 1, 1);

					ChangeImgName(imgfile, fileid);
			...
			try
			{
				...
				if (m_pPage != NULL)
				{
					...
					sprintf(c_imgfile, "%s%d.bmp", imgfile, fileid);
					...
					char pngFile[500] = { 0 };
					sprintf(pngFile, "%s%d.png", imgfile, fileid);
					mImage.Save(CharToCString(pngFile));
...
int _tmain(int argc, _TCHAR* argv[])
{
	if (argc == 5)
	{
		...
		int status = ConvertPdfToImg(c_pdffile, c_pdfsave, fileid);

		GetNowTime(now);
		printf("end %s \n", now);

		PostThreadMessage(nthreadid, WM_MSG_PDF2IMG_STATUS, status, 0);
