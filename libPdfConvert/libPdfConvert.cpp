// libPdfConvert.cpp : 定义 DLL 的初始化例程。
//
...
extern "C" {
	...
	int WINAPI StartConvertPdfToTxt(const char *file, const char *outpath, const int &fileid, const int &minPage, int &pageNum, const int &txtSize, const int &timeout)
	{
		AFX_MANAGE_STATE(AfxGetStaticModuleState());
		...
		sprintf_s(paramstr, 2000, "pdftotxt.exe %s %s %d %d %d %d", file, outpath, nthreadId, fileid, minPage, txtSize);
	...
	int WINAPI StartConvertPdfToImg(const char *file, const char *outpath, const int &fileid, const int &timeout)
	{
		AFX_MANAGE_STATE(AfxGetStaticModuleState());
		...
		sprintf_s(paramstr, 2000, "pdftoimg.exe %s %s %d %d", file, outpath, nthreadId, fileid);
	...
	void WINAPI StartConvertPdfToTxtPng(const char *file, const char *outtxtpath, const char *outimgpath, const int &fileid, const int &isoriginal, const int &minPage, int &pageNum, const int &txtSize, const int &timeout, const int &istoimg, int &txtstatus, int &pngstatus)
	{
		AFX_MANAGE_STATE(AfxGetStaticModuleState());
		...
		sprintf_s(paramstr, 2000, "pdftotxtpng.exe %s %s %s %d %d %d %d %d %d", file, outtxtpath, outimgpath, nthreadId, fileid, minPage, txtSize, istoimg, isoriginal);
