...
bool ChangeImgName(string imgPath, string outfile)
{
...
// 
// 			char newname[200] = { 0 };
// 			sprintf(newname, "%s%d.png", imgPath.c_str(), fileid);
	...
	if (jpgNum>1)
	{
		DeleteDirectory(CharToCString(imgPath.c_str()));
	}
...
void ExecuteScaleImg(char *sourcefile, char *outfile, int type)
{
	char paramstr[1024] = { 0 };
	STARTUPINFOA si = { sizeof(si) };
	PROCESS_INFORMATION pi;
	...
	sprintf(paramstr, "ScaleImg.exe %s %s %d", sourcefile, outfile, type);
...
int ConvertPdfToTxt(const char *sourcefile, const char *outtxtpath, const char *outpngpath, const int &fileid, const int &minpage, const int &txtSize, int &pageNum, const int &istoimg, const int &isoriginal)
{
	...
	int nToPngStatusOk = 0;
	int nToTxtStatusError = 0;
	int nToPngStatusError = 0;
	...
	{
		pageNum = pFPdfGetPageCount(pDoc);

		if (!isoriginal && pageNum < minpage)
		{
			pFpdfCloseDocument(pDoc);
			return PageLess;
		}
		...
		//toimg
		if (istoimg && nToTxtStatusOk)
		{
			try
			{
				char outimgfile[1024] = {};
				sprintf_s(outimgfile, 1024, "%s%d.png", imgpath, fileid);
				char outsmallfile[1024] = {};
				sprintf_s(outsmallfile, 1024, "%s%d_small.png", imgpath, fileid);
				if (bIsUnrecognizable)
				{
					char imgpath1[1024] = {};//通过pdf2png.exe转换的目录
					sprintf_s(imgpath1, 1024, "%s%d\\", imgpath, fileid);
					int bhtml = ExtractPdftoPng(sourcefile, imgpath1, 1, 1);
				...
				m_pPage = pFPDFLoadPage(pDoc, 0);

				if (m_pPage != NULL)
				{
					double nwidth = pFPDFGetPageWidth(m_pPage);
					double nheight = pFPDFGetPageHeight(m_pPage);
					double scale;
					...
					void *imgbuf = NULL;
					FPDF_BITMAP bitmap = pFPDFBitmapCreate(width, height, FALSE);
					pFPDFBitmapFillRect(bitmap, 0, 0, width, height, 255, 255, 255, 255);
					pFPDFRenderPageBitmap(bitmap, m_pPage, 0, 0,
						width, height, 0,
						FPDF_ANNOT | FPDF_LCD_TEXT | FPDF_NO_GDIPLUS);
					imgbuf = pFPDFBitmapGetBuffer(bitmap);

					char c_imgfile[500] = { 0 };
					sprintf(c_imgfile, "%s%d.bmp", imgpath, fileid);
					BMP_SaveFile(c_imgfile, imgbuf, width, height);
					// 				delete imgbuf;

					pFPDFBitmapDestroy(bitmap);

					CImage mImage;
					mImage.Load(CharToCString(c_imgfile));
					mImage.Save(CharToCString(outimgfile));
...
int _tmain(int argc, _TCHAR* argv[])
{
	if (argc == 10)
	{
		...
		char strimgpath[STRSIZE] = {};
		int fileid;
		int minpage;
		int txtsize;
		int istoimg;
		int isoriginal;
		wchar_t temp[STRSIZE] = { 0 };
		...
		strcpy(strtxtpath, ctemp);
		delete[] ctemp;

		memset(temp, 0, STRSIZE);
		wcscpy(temp, argv[3]);
		ctemp = WCharToChar(temp, wcslen(temp));
		strcpy(strimgpath, ctemp);
		delete[] ctemp;
